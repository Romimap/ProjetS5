...
CREATE PROCEDURE p_duree_evenement(IN id_event INT(10))
BEGIN
    SELECT (DATEDIFF(date_fin, date_debut) + 1) AS 'duree_evenement'
    FROM evenement
    WHERE id = id_event;
END |

CREATE PROCEDURE p_inscription_count(IN id_event INT(10))
BEGIN
...
CREATE PROCEDURE p_is_admin (IN id_m INT(10))
BEGIN
    SELECT COUNT(*) AS 'est_admin' FROM membres WHERE role = 'Admin' AND id = id_m;
END |

CREATE PROCEDURE p_is_contributeur (IN id_m INT(10))
BEGIN
    SELECT COUNT(*) AS 'est_contributeur' FROM membres WHERE role = 'Contributeur' AND id = id_m;
END |
